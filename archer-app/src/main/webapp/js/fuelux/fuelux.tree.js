...
(function (factory) {
	...
	Tree.prototype = {
		...
		clickMenu : function($el,op,data){
			var menuDefults={
				title: '操作',
				data:data
			}
			$.extend(true, menuDefults, op);
			$el.bind("mousedown",function (e){
				if(e.which==3){
					$(this).contextPopup(menuDefults);
				}			
			});
		},
			...
			this.options.dataSource( treeData ? treeData : {} , function (items) {
				...
				$.each( items.data, function(index, value) {
					...
					if(value.type === 'folder') {
						...
						if(self.options.menuOptions){//绑定右键菜单
							self.clickMenu($entity.children("div[class=tree-branch-header]"),self.options.menuOptions,value);
						}
						...
						if(self.options.menuOptions){//绑定右键菜单
							self.clickMenu($entity,self.options.menuOptions,value);
						}		
					}
