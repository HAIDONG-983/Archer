...
(function ($) {
    ...
    /**
     * var jQuery = funciton(selector,context){
 ...
 * }
    ...
    $.fn.jqGrid = function (pin) {
        ...
        return this.each(function () {
            ...
            var i, dir;
            if (this.p.colNames.length === 0) {//TODO BUG若name为汉语，无法填充数据
            ...
            var gv = $("<div class='ui-jqgrid-view'></div>"),
                isMSIE = $.jgrid.msie;
            ts.p.direction = $.trim(ts.p.direction.toLowerCase());//表格文字方向
            ...
            var sortkeys = ["shiftKey", "altKey", "ctrlKey"],
                ...
                formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata) {
                    ...
                    var cm = ts.p.colModel[pos],
                        ral = cm.align ? cm.align : "center", result = "style=\"", clas = cm.classes, nm = cm.name, celp, acp = [];
                    if (ral) {
                ...
                formatter = function (rowId, cellval, colpos, rwdat, _act) {
                    ...
                    if (cm.formatter !== undefined) {//自定义formatter方法是否存在
                    ...
                    } else if (cm.edittype === "select") {//对编辑类型是select的单元格进行自定义的格式化
                        if (typeof cellval == "number") {
                            cellval = cellval + "-" + EnumConstants[cm.editoptions.value][cellval][1];
                        }
                        v = cellVal(cellval);
                    } else {
                        v = cellVal(cellval);
                ...
                populate = function (npage) {//填充数据
                    if (!ts.grid.hDiv.loading) {
                        ...
                        prm["start"] = ts.p.rowNum * (ts.p.page - 1);
            ...
            for (i = 0; i < this.p.colNames.length; i++) {
                ...
                //var thid=ts.p.colModel[i].index?ts.p.colModel[i].index:ts.p.colModel[i].name;
            ...
            //$(this).triggerHandler('resize.jqGrid');//trigger window resize to make the grid get the correct size
        });
    };
    $.jgrid.extend({
        ...
        getSelectedRowDatas:function () {
            ...
            this.each(function(){
                var $t = this;
                var ids=$($t).jqGrid('getGridParam','selarrrow');
                $.each(ids,function (i, n) {
                    var rowData = $($t).jqGrid("getRowData",n);
                    srds[srds.length]=rowData;
                });
            });
            return srds;
        },
        getRowObject:function(name){
            var obj = null;
            this.each(function(){
                var $t = this;
                var id=$($t).jqGrid('getGridParam','selrow');
                var rowData = $($t).jqGrid('getRowData',id);
                if(rowData[name]!=undefined)obj=rowData[name];
            });
            return obj;
        },
...
(function ($) {
    ...
    $.fn.fmatter.actions = function (cellval, opts) {
        var op = {keys: false, editbutton: false, delbutton: false, editformbutton: false,custome:false},
        ...
        if (op.custome) {
            window["customeIconButton"]=op.customeOptions;
            $.each(op.customeOptions,function (i,option) {
                ocl = "id='jCustomButton_" + rowid + "_"+i+"' onclick=window.customeIconButton['"+i+"']['callback'].call(this,"+rowid+") onmouseover=jQuery(this).addClass('ui-state-hover'); onmouseout=jQuery(this).removeClass('ui-state-hover'); ";
                str += "<div title='" + option.title + "' style='float:left;margin-left:5px;' class='ui-pg-div ui-inline-del' " + ocl + "><span class='"+option.icon+"'></span></div>";
                //$("#jCustomButton_"+rowid + "_"+i).click(option['callback']);
            });
    ...
    $.fn.fmatter.button = function (cellval, opts,rwd) {
        ...
        return "<div style='margin-left:-13px;'>" + str + "</div>";;
                ...
                case "select" :
                    ...
                    if (options.dataUrl !== undefined) {
                        ...
                        if (typeof options.value === 'string') {
                            //重写源码   option.value===string时，默认从枚举常量中获取下拉
                            var enmuArray = EnumConstants[options.value] || [];
                            $.each(enmuArray, function (n, v) {
                                ov = document.createElement("option");
                                ov.setAttribute("role", "option");
                                ov.value = v[0];
                                ov.innerHTML = v[0] + "-" + v[1];
                                elem.appendChild(ov);
                            });
    ...
    $.jgrid.extend({
        ...
        editGridRow: function (rowid, p) {
            p = $.extend(true, {
                ...
                drag: true,
                resize: true,
                url: null,//数据保存的url，如果没有设置,默认取Grid配置参数中的editurl
                mtype: "POST",
                clearAfterAdd: true,
                closeAfterEdit: true,
                closeAfterAdd: true,
                ...
                beforeInitData: null,//在初始化输入表单数据前触发
                beforeShowForm: null,//表单显示前触发
                afterShowForm: null,
                beforeSubmit: null,//表单提交前触发
                afterSubmit: null,
                afterError:null,//后台处理失败执行方法
                onclickSubmit: null,
                afterComplete: null,
                onclickPgButtons: null,
                afterclickPgButtons: null,
                editData: {},//发送到服务器的附加数据，键值对对象
                recreateForm: false,
                jqModal: true,
                closeOnEscape: false,
                addedrow: "first",
            ...
            return this.each(function () {
                ...
                function createData(rowid, obj, tb, maxcols) {
                    var nm,//colModel中的name
                        hc, trdata, cnt = 0, tmp, dc, elc, retpos = [], ind = false,
                    ...
                    $(obj.p.colModel).each(function (i) {
                        ...
                        if (nm !== 'cb' && nm !== 'subgrid' && (this.editable === true || rowid == '_empty') && nm !== 'rn' && this.formatter !== "actions" && this.addable != false) {
                            if (ind === false) {
                                tmp = "";
                            } else {
                            ...
                            var opt = $.extend({}, this.editoptions || {}, {id: nm, name: nm}),//修改条件
                                frmopt = $.extend({}, {
                                    elmprefix: '',
                                    elmsuffix: '',
                                    rowabove: false,
                ...
                function fillData(rowid, obj, fmid) {
                    ...
                    $('td[role="gridcell"]', tre).each(function (i) {
                        nm = cm[i].name;
                        // hidden fields are included in the form //&& cm[i].editable===true
                        if (nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
                            if (nm === obj.p.ExpandColumn && obj.p.treeGrid === true) {
                                tmp = $(this).text();
                            } else {
                ...
                function postIt() {//提交数据
                    ...
                    if (ret[0] && !rp_ge[$t.p.id].processing) {
                        ...
                        var ajaxOptions = $.extend({
                            ...
                            data: $.isFunction(rp_ge[$t.p.id].serializeEditData) ? rp_ge[$t.p.id].serializeEditData.call($t, postdata) : postdata,//是否提交自己的序列化数据
                ...
                var rtlb = $t.p.direction === "rtl" ? true : false,
                    bp = rtlb ? "nData" : "pData",
                    bn = rtlb ? "pData" : "nData";
                createData(rowid, $t, tbl, maxCols);//创建数据
                // buttons at footer 数据表格页脚按钮(提交，取消)
                ...
                $.jgrid.style_edit_form($("#" + frmgr));//自定义表单样式
        ...
        delGridRow: function (rowids, p) {
            ...
            return this.each(function () {
                ...
                if ($("#" + $.jgrid.jqID(IDs.themodal))[0] !== undefined) {
                    ...
                    $.jgrid.style_edit_form($("#" + dtbl));//自定义表单样式
                    $.jgrid.viewModal("#" + $.jgrid.jqID(IDs.themodal), {
                        gbox: "#gbox_" + $.jgrid.jqID(gID),
                        jqm: rp_ge[$t.p.id].jqModal,
                        overlay: rp_ge[$t.p.id].overlay,
        ...
        navGrid: function (elem, o, pEdit, pAdd, pDel, pSearch, pView) {
            o = $.extend({
                ...
                del: true,
                delicon: "ce-icon fa fa-trash-o red",
                search: true,
        ...
        saveRow: function (rowid, successfunc, url, extraparam, aftersavefunc, errorfunc, afterrestorefunc) {
            ...
            if (editable === "1") {
                ...
                $('td[role="gridcell"]', ind).each(function (i) {
                    cm = $t.p.colModel[i];
                    nm = cm.name;
                    //&& cm.editable===true && !$(this).hasClass('not-editable-cell')
                    if (nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && cm.formatter !== "actions") {
                        switch (cm.edittype) {
                            case undefined :
                                tmp[nm] = $(this).html() !== "&nbsp;" ? $(this).html() : "";
                                break;
                            //alert($(this).html());
                            case "checkbox":
                                var cbv = ["Yes", "No"];
